...
export function TodayChip({ count, target, source }: { count: number | null; target: number; source: 'personal' | 'default' }) {
...
export function TodayPanel({ count, target, groups, showGroups }: { count: number | null; target: number; groups: StockCount[]; showGroups: boolean }) {
  const copy = progressCopy(count, target)

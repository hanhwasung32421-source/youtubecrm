import { ApiFail, apiError, authenticate, cleanText, noStoreJson, readJson, requireUuid } from '@/lib/v3/server'
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return noStoreJson({ ok: true, series: { id: data.id, name: data.name, stockName: data.stock_name } })
  } catch (e) {
